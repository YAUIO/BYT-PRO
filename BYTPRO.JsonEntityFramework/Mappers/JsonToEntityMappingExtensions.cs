...
namespace BYTPRO.JsonEntityFramework.Mappers;

public static class JsonToEntityMappingExtensions
{
    public static object? MapToEntity(this List<JsonElement> fields, Type target, object? uow)
    {
        ...
        if (parameters != null)
        {
            for (var i = 0; i < parameters.Length; i++)
            {
                ...
                if (prop == null || prop.IsDefined(typeof(JsonIgnoreAttribute), false))
                {
                    ...
                    {
                        throw new InvalidOperationException($"No service found for type '{param.ParameterType}, name {param.Name}, services {uow}'");
                    }
                    continue;
                }
        ...
        else
        {
            ...
            for (var i = 0; i < props.Length; i++)
            {
                ...
                if (prop.IsDefined(typeof(JsonIgnoreAttribute), false))
                {
                    ...
                    {
                        throw new InvalidOperationException($"No service found for type '{prop.PropertyType}, name {prop.Name}, services {uow}'");
                    }
                    continue;
                }
